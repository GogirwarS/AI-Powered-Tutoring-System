...
def chapter_test(course_name, chapter_id):
    ...
    if mcq_data is None or course_name not in mcq_data:
        flash('Error loading test data. Please try again later.', 'danger')
        return redirect(url_for('course', course_name=course_name))

    chapter_key = f"chapter{chapter_id}"
    if chapter_key not in mcq_data[course_name]["chapters"]:
        flash('Error loading test data. Please try again later.', 'danger')
        return redirect(url_for('course', course_name=course_name))

    # Fetch the questions from the MCQ data
    questions = mcq_data[course_name]["chapters"][chapter_key]["questions"]
    chapter_title = mcq_data[course_name]["chapters"][chapter_key].get("title", "Chapter Title Not Found")
    ...
    if request.method == 'POST':
        # Handle the POST request (user answers)
        answers = {}
        for question in questions:
            if "id" in question:
                answers[str(question["id"])] = request.form.get(f"question_{question['id']}")
        ...
        if passed:
            ...
            course_progress = CourseProgress.query.filter_by(user_id=current_user.id, course_name=course_name).first()
            if course_progress and course_progress.current_chapter == chapter_id:
                course_progress.current_chapter += 1
                if course_progress.current_chapter > len(mcq_data[course_name]['chapters']):
                    course_progress.completed = True
            db.session.commit()

            flash(f'Test passed! Score: {score:.2f}%', 'success')
            return redirect(url_for('chapter', course_name=course_name, chapter_id=chapter_id + 1))
        else:
    ...
    # If GET request, render the test page
    return render_template('test.html', course_name=course_name, chapter_id=chapter_id, questions=questions, chapter_title=chapter_title)
